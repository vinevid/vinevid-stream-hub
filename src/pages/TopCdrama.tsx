...
const fetchTopCdramaVideos = async () => {
...
const TopCdrama = () => {
  const { data: videos = [] } = useQuery({
    queryKey: ["top-cdrama-videos"],
    queryFn: fetchTopCdramaVideos,
  });
  ...
  return (
    <>
      <Helmet>
        <title>Top CDrama | VineVid</title>
        <meta name="description" content="Top Chinese drama series on VineVid. Best CDrama content." />
        <link rel="canonical" href={`${location.origin}/top-cdrama`} />
      </Helmet>
      <Header />
      <main className="container py-8">
        <div className="space-y-6">
          <div className="text-center space-y-2">
            <h1 className="text-3xl font-bold tracking-tight">Top CDrama</h1>
            <p className="text-muted-foreground">
              Best Chinese drama series on VineVid
            </p>
          </div>
          ...
          {videos.length > 0 ? (
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">No top CDrama series found.</p>
            </div>
          )}
        </div>
      </main>
