...
create type if not exists public.comment_status as enum ('pending','approved','spam');
...
create policy if not exists "Public read posters"
...
create policy if not exists "Admins manage posters"
...
create policy if not exists "Admins manage site assets"
  on storage.objects for all to authenticated
